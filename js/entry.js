...
function setup() {
  ...
  setRuleFromString(presets['Life']['Conway\'s Game of Life']);

  // Before the simulation runs, pre-populate the canvas with some data
  drawPattern();
}
  ...
  export function resetTextureSizes() {
    ...
    if(
      ...
      canvas.clientWidth / window.devicePixelRatio == variables.canvas.width.value &&
      canvas.clientHeight / window.devicePixelRatio == variables.canvas.height.value
    ) return;

    // Resize the canvas
    renderer.setSize(
      variables.canvas.maximized ? window.innerWidth : variables.canvas.width.value / window.devicePixelRatio,
      variables.canvas.maximized ? window.innerHeight : variables.canvas.height.value / window.devicePixelRatio
    );

    variables.canvas.width.value = canvas.clientWidth * window.devicePixelRatio;
    variables.canvas.height.value = canvas.clientHeight * window.devicePixelRatio;
